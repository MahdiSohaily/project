<?php
$pageTitle = "ویرایش انتقال به انبار";
...
if (isset($_POST['selected_record_id'])) {
    ...
    $selected_record = getRecord($record_id);
    $transfer_record = getTransferRecord($record_id);
    $affected_record = $transfer_record['affected_record'];


    if (getPurchaseReportById($affected_record)) {
        $allowedAmount = getPurchaseReportById($affected_record)[0]['remaining_qty'] + $selected_record['purchase_quantity'];
    } else {
        $allowedAmount = $selected_record['purchase_quantity'];
    }

    $purchase_quantity = $_POST['purchase_quantity'];

    if (intval($purchase_quantity) > $allowedAmount) {
    ...
    updateTransferRecord($transfer_record['id'], $_POST);
    updateExitRecord($transfer_record['exit_id'], $_POST);
...
if (isset($_POST['delete_purchase'])) {
    ...
    deleteExitRecord($transfer_record['exit_id']);
    deleteTransferRecord($transfer_record['id']);
...
<body style="padding: 0 !important;">
    ...
    <form id="edit" method="post" action="<?= htmlspecialchars($_SERVER['PHP_SELF']) ?>" class="bg-gray-200 rounded-lg mt-3 p-5 w-full grid grid-cols-4 gap-3">
        <div>
...
function updateTransferRecord($record_id, $data)
{
    $purchase_quantity = $data['purchase_quantity'];
    $stock_edit = $data['stock_edit'];

    global $stock;
    $statement = PDO_CONNECTION->prepare("UPDATE $stock.transfer_record 
                                        SET quantity = :purchase_quantity,
                                        stock = :stock_edit
                                        WHERE id = :record_id");
    $statement->bindParam(':purchase_quantity', $purchase_quantity);
    $statement->bindParam(':stock_edit', $stock_edit);
...
function updateExitRecord($record_id, $data)
{
    global $stock;
    $purchase_quantity = $data['purchase_quantity'];

    $statement = PDO_CONNECTION->prepare("UPDATE $stock.exitrecord SET qty = :purchase_quantity WHERE id = :record_id");
    $statement->bindParam(':purchase_quantity', $purchase_quantity);
    $statement->bindParam(':record_id', $record_id);
    $statement->execute();
}

function getTransferRecord($id)
{
    global $stock;
    $sql = "SELECT * FROM $stock.transfer_record WHERE qtybanck_id = :id";
...
function deleteExitRecord($record)
{
    global $stock;
...
{
    global $stock;
    $statement = PDO_CONNECTION->prepare("DELETE FROM $stock.transfer_record WHERE id = :record_id");
    $statement->bindParam(':record_id', $record);
    $statement->execute();
}
